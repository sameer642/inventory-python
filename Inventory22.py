...
def Database():
    global conn, cursor
    conn = sqlite3.connect('sameer22.db')
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS `sameer` (sameer_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username TEXT, password TEXT)")
    cursor.execute("CREATE TABLE IF NOT EXISTS `product` (product_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, product_name TEXT, product_qty TEXT, product_price TEXT)")
    cursor.execute("SELECT * FROM `sameer` WHERE `username` = 'sameer' AND `password` = 'sameershah'")
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `sameer` (username, password) VALUES('sameer', 'sameershah')")
        conn.commit()
...
def Login(event=None):
    ...
    if USERNAME.get == "" or PASSWORD.get() == "":
        ...
        cursor.execute("SELECT * FROM `sameer` WHERE `username` = ? AND `password` = ?",
